...
const start = async () => {
  ...
  try {
    ...
    process.on('SIGINT', () => natsWrapper.client.close());//close the connection when client is shut, otherwise nats thinks the connection is up and sends events to it
    process.on('SIGTERM', () => natsWrapper.client.close());  
    
    new OrderCreatedListener(natsWrapper.client).listen();//create listener instance and start listen
