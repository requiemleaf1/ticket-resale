...
declare global {// declare "signin" as a function in global scope that returns a promise which resolves with value of string[]
  var signin: (id?: string) => string[];// so it can be called in different services without having to be imported to diffirent files
...
global.signin = (id? : string) => {// implement the global signin function as a helper function
