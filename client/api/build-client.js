...
export default ({ req }) => {
  ...
  // requests should be made to http://ingress-nginx.ingress-nginx...laksdjfk
    ...
    return axios.create({//return a pre-configured axios instance with the specific property
      baseURL:
        'http://www.ticket-resale2024.college/',// if we are on server, it is running in the container, so the localhost location is different than the local mechine.we need to manualy refer to the ingress -srv to append domain to the url
      headers: req.headers,
    });
  } else {
